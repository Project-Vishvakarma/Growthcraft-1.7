...
public class RenderBamboo implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
        if (modelId == id) {
            ...
            renderer.setRenderBounds(0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D);
            IIcon icon = BlockBambooStalk.tex[0];
            renderer.renderFaceYPos(block, x, (float) y, z, icon);
            renderer.renderFaceYNeg(block, x, (float) y, z, icon);

            final float d = 0.75F;
            tessellator.setColorOpaque_F(f * f1 * d, f * f2 * d, f * f3 * d);
            renderer.setRenderBounds(0.25D, 0.0D, 0.25D, 0.75D, 1.0D, 0.75D);
            icon = BlockBambooStalk.tex[2];
            renderer.renderFaceZNeg(block, x, (float) y, z, icon);
            renderer.renderFaceZPos(block, x, (float) y, z, icon);
            renderer.renderFaceXNeg(block, x, (float) y, z, icon);
            renderer.renderFaceXPos(block, x, (float) y, z, icon);
            renderer.setRenderBounds(0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D);
